...
def plot_feature_importance(model, stock, data_stocks):
...
for stock in stocks:
    ...
    data_stocks_filtered, fea = plot_feature_importance(model, stock,data)
...
for stock in stocks:
    ...
    def split_train_val_testraw(df,stock, dataloc, tr = 0.8, vl = 0.1, h = 1, l = 10,fea = 19, pred = 1, plotcheck=False):
...
# สร้าง dictionary เพื่อเก็บข้อมูลหุ้นแต่ละตัว
data_stocks = {}
del df_result
...
for stock in stocks:
    ...
    tanh_coeff = 15          # ลดความรุนแรงของฟังก์ชัน tanh
    z_dim = 48               # เพิ่มความซับซ้อนใน latent space
    hid_d = 48               # เพิ่มจำนวน hidden units ใน discriminator
    hid_g = 48               # เพิ่มจำนวน hidden units ใน generator
    checkpoint_epoch = 5    # บันทึกโมเดลบ่อยขึ้น
    batch_size = 32          # ลดขนาด batch เพื่อการเรียนรู้ละเอียดขึ้น
    n_epochs = 500         # เพิ่มรอบการเทรน
    tr = 0.5                 # คง train set ที่ 80%
    vl = 0.1                 # เพิ่ม validation set เป็น 20%
...
for stock in stocks:
    ...
    tanh_coeff = 20          # ลดความรุนแรงของฟังก์ชัน tanh
    z_dim = 64               # เพิ่มความซับซ้อนใน latent space
    hid_d = 64               # เพิ่มจำนวน hidden units ใน discriminator
    hid_g = 64               # เพิ่มจำนวน hidden units ใน generator
    checkpoint_epoch = 5    # บันทึกโมเดลบ่อยขึ้น
    batch_size = 16          # ลดขนาด batch เพื่อการเรียนรู้ละเอียดขึ้น
    n_epochs = 600         # เพิ่มรอบการเทรน
    tr = 0.5                 # คง train set ที่ 80%
    vl = 0.1                 # เพิ่ม validation set เป็น 20%
...
for stock in stocks:
    ...
    tanh_coeff = 25          # ลดความรุนแรงของฟังก์ชัน tanh
    z_dim = 80               # เพิ่มความซับซ้อนใน latent space
    hid_d = 80               # เพิ่มจำนวน hidden units ใน discriminator
    hid_g = 80               # เพิ่มจำนวน hidden units ใน generator
    checkpoint_epoch = 5    # บันทึกโมเดลบ่อยขึ้น
    batch_size = 32          # ลดขนาด batch เพื่อการเรียนรู้ละเอียดขึ้น
    n_epochs = 500         # เพิ่มรอบการเทรน
    tr = 0.5                 # คง train set ที่ 80%
...
for stock in stocks:
    ...
    tanh_coeff = 30          # ลดความรุนแรงของฟังก์ชัน tanh
    z_dim = 96               # เพิ่มความซับซ้อนใน latent space
    hid_d = 96               # เพิ่มจำนวน hidden units ใน discriminator
    hid_g = 96               # เพิ่มจำนวน hidden units ใน generator
    checkpoint_epoch = 5    # บันทึกโมเดลบ่อยขึ้น
    batch_size = 64          # ลดขนาด batch เพื่อการเรียนรู้ละเอียดขึ้น
    n_epochs = 300         # เพิ่มรอบการเทรน
    tr = 0.5                 # คง train set ที่ 80%
    vl = 0.1                 # เพิ่ม validation set เป็น 20%
...
for stock in stocks:
    ...
    tanh_coeff = 10          # ลดความรุนแรงของฟังก์ชัน tanh
    z_dim = 64               # เพิ่มความซับซ้อนใน latent space
    hid_d = 32               # เพิ่มจำนวน hidden units ใน discriminator
    hid_g = 64               # เพิ่มจำนวน hidden units ใน generator
    checkpoint_epoch = 5    # บันทึกโมเดลบ่อยขึ้น
    batch_size = 16          # ลดขนาด batch เพื่อการเรียนรู้ละเอียดขึ้น
    n_epochs = 500         # เพิ่มรอบการเทรน
...
for stock in stocks:
    ...
    tanh_coeff = 20          # ลดความรุนแรงของฟังก์ชัน tanh
    z_dim = 32               # เพิ่มความซับซ้อนใน latent space
    hid_d = 80               # เพิ่มจำนวน hidden units ใน discriminator
    hid_g = 48               # เพิ่มจำนวน hidden units ใน generator
    checkpoint_epoch = 5    # บันทึกโมเดลบ่อยขึ้น
    batch_size = 32          # ลดขนาด batch เพื่อการเรียนรู้ละเอียดขึ้น
...
import json
import joblib
# stocks = ["BBL","KBANK", "MINT", "LH", "CPALL"]
stocks = [st]
...
# # ค่าของ RMSE และ SR จากโมเดล 8 ตัว (ต้องใส่ค่าจริง)
# rmse_values = np.array([rmse_ts,rmse_ts_para2, rmse_xg, rmse_xg_para2, rmse_fin, rmse_fin2, rmse_facfin, rmse_facfincut])  # RMSE ของแต่ละโมเดล
# sr_values = np.array([sr_ts, sr_ts_para2, sr_xg, sr_xg_para2, sr_fin, sr_fin2, sr_facfin, sr_facfincut])  # Sharpe Ratio ของแต่ละโมเดล

# # --- 1. Normalize RMSE และ SR ให้อยู่ในช่วง 0-1 ---
# normalized_rmse = 1 / rmse_values  # ใช้ค่ากลับของ RMSE เพื่อให้ค่าน้อยได้ Weight มาก
# normalized_rmse = normalized_rmse / np.sum(normalized_rmse)  # Normalize ให้อยู่ในช่วง 0-1

# normalized_sr = sr_values / np.sum(sr_values)  # Normalize SR ให้อยู่ในช่วง 0-1

# # --- 2. คำนวณน้ำหนักรวมจาก RMSE และ SR ---
# weights = normalized_sr / normalized_rmse  # SR สูง และ RMSE ต่ำ จะได้ Weight สูง
# weights = weights / np.sum(weights)  # Normalize น้ำหนักรวมให้อยู่ในช่วง 0-1

# # --- 3. ใช้ Weighted Voting ---
# predictions = np.array([sgn_fake_timeseries, sgn_fake_timeseries_para2, sgn_fake_xgboost, sgn_fake_xgboost_para2, sgn_fake_fingan, sgn_fake_fingan_para2, sgn_fake_facfin, sgn_fake_facfin_cut])  # รวมค่าทำนายของโมเดลทั้งหมด
# Majority_final_prediction = np.sign(np.sum(predictions, axis=0))  

# weighted_predictions = np.dot(weights, predictions)  # ใช้ Weight คูณค่าทำนาย
# Weighted_final_prediction = np.sign(weighted_predictions)  # ใช้ sign() ตัดสินทิศทาง
...
for stock in stocks:
    ...
    stock_df = stock_df.dropna().reset_index(drop=True)
    X_value = pd.DataFrame(stock_df.iloc[:, :-1])
    print(f"X_value = {X_value.info()}")
    print(f"X_value = {X_value.head(10)}")
    ...
    X_scale_dataset = pd.DataFrame(X_scale_dataset)
    dataset = X_scale_dataset
    dataset['nret'] = pd.DataFrame(stock_df.iloc[:, -1:])
    ...
    num_lags = 10  # จำนวนวันย้อนหลัง

    for lag in range(1, num_lags + 1):
        train_df[f'Close_Lag_{lag}'] = train_df['nret'].shift(lag)

    for lag in range(1, num_lags + 1):
        valid_df[f'Close_Lag_{lag}'] = valid_df['nret'].shift(lag)

    for lag in range(1, num_lags + 1):
        test_df[f'Close_Lag_{lag}'] = test_df['nret'].shift(lag)

    train_df = train_df.dropna().reset_index(drop=True)
    valid_df = valid_df.dropna().reset_index(drop=True)
    test_df = test_df.dropna().reset_index(drop=True)

    y_train = train_df['nret'].copy()
    X_train = train_df.drop(columns=['nret'])

    y_valid = valid_df['nret'].copy()
    X_valid = valid_df.drop(columns=['nret'])

    y_test  = test_df['nret'].copy()
    X_test  = test_df.drop(columns=['nret'])

import numpy as np
from sklearn.metrics import mean_squared_error

def calculate_pointwise_rmse(predictions, y_true):
    squared_errors = (predictions - y_true) ** 2  # คำนวณ (y_true - y_pred)^2
    pointwise_rmse = np.sqrt(squared_errors)  # ถอดรูทเพื่อให้เป็น RMSE
    return pointwise_rmse  # Shape (8, N)

# --- ค่าทำนายจาก 8 โมเดล ---
# sgn_fake8 = sgn_fake
# forecast8 = forecast
predictions = np.array([forecast1, forecast2, forecast3, 
                        forecast4, forecast5, forecast6, 
                        forecast7, forecast6])  # Shape (8, N)

y_true = y_test  # ค่าจริง, Shape (N,)
y_true = np.array(y_true)  # แปลงเป็น numpy array
y_true = y_true[np.newaxis, :]  # ขยายมิติเป็น (1, 938)

# --- คำนวณ RMSE แบบ Dynamic ---
rmse_values = calculate_pointwise_rmse(predictions, y_true)  # Shape (8, N)


def calculate_cumulative_profit_for_return(predictions, y_true):
    """
    คำนวณกำไรสะสม (Cumulative Profit) สำหรับการทำนายทิศทางโดยใช้ค่า return จริง
    :param predictions: ค่าทำนายทิศทางของแต่ละโมเดล, shape = (8, N)
    :param y_true: ค่า return จริง, shape = (N,)
    :return: กำไรสะสมแบบ Dynamic, shape = (N,)
    """
    # คำนวณกำไรสำหรับแต่ละจุดเวลา
    # กำไร = ค่าผลตอบแทนจริง * ทิศทางที่ทำนาย (1 ถ้าทำนายถูก, 0 ถ้าผิด)
    profit = np.where(predictions == 1, y_true, 0)  # กำไรเมื่อทำนายขึ้นและจริงขึ้น
    profit += np.where(predictions == -1, y_true, 0)  # กำไรเมื่อทำนายลงและจริงลง

    # คำนวณกำไรสะสม (Cumulative Profit)
    cumulative_profit = np.cumsum(profit, axis=1)  # คำนวณผลรวมสะสมตามแกนเวลา (axis=1)
    
    return cumulative_profit  # Shape (8, N)

# --- ค่าทำนายจาก 8 โมเดล ---
predictions_sgn = np.array([sgn_fake1, sgn_fake2,sgn_fake3,sgn_fake4,sgn_fake5,sgn_fake6,sgn_fake7,sgn_fake8])  # Shape (8, N)


summed_predictions = np.sum(predictions_sgn, axis=0)  # หาผลรวมในแต่ละคอลัมน์

# --- 2. กรณีผลรวมเป็น 0, เลือกค่า '1' หรือ '-1' หรือทำการสุ่ม ---
majority_vote = np.sign(summed_predictions)  # ถ้าผลรวม > 0 จะได้ 1 ถ้าผลรวม < 0 จะได้ -1
majority_vote[summed_predictions == 0] = np.random.choice([-1, 1], size=np.sum(summed_predictions == 0))  # กรณีผลรวมเป็น 0 ให้สุ่ม

# --- 3. แสดงผลลัพธ์ ---
print("Shape of majority_vote:", majority_vote.shape)  # ควรเป็น (938,)

# stocks = "BBL"


df = pd.DataFrame(majority_vote, columns=['majority_vote'])

# บันทึกเป็นไฟล์ CSV
df.to_csv(f'majority_vote_{stock}.csv', index=False)
# print(majority_vote)  # ผลลัพธ์ที่ได้
